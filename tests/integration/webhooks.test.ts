...
describe('POST /webhooks/slate', () => {
  it('accepts valid Slate payloads asynchronously', async () => {
    const { validSlatePayload } = await import('../fixtures/payloads.js')
    const response = await postJson('/webhooks/slate', validSlatePayload)
    const body = await readJson<{ ingestionId: string }>(response)
    ...
    expect(ingestion.status).toBe('completed')
    expect(ingestion.result?.sourceSystem).toBe('SLATE')
  })
})
